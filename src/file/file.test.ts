...
test("fileUpload posts multipart without json content type", async () => {
  ...
  await fileUpload(client, { type: "voucher", filename: "invoice.pdf", data: new Blob(["pdf"]) })
...
test("fileDownload returns filename from content disposition", async () => {
  const response = new Response(new Uint8Array([1, 2, 3]), {
    headers: { "Content-Disposition": "attachment; filename*=UTF-8''rechnung.pdf", "Content-Type": "application/pdf" },
  })
