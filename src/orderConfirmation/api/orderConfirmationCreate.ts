...
import { type OrderConfirmationBody, orderConfirmationBodySchema } from "../schema/orderConfirmationSchemas.js"

export async function orderConfirmationCreate(
  client: LexwareClient,
  input: OrderConfirmationBody,
): PromiseResult<LexwareUnknownResponse> {
  const op = "orderConfirmationCreate"
  const r = a.safeParse(orderConfirmationBodySchema, input)
